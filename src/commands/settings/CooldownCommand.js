  ...
  async run(message, args) {
    ...
    switch (option) {
      case 'set':
        ...
        if (!data.cooldown[command]) return message.fail('Указанной вами команды не существует либо у вас недостаточно прав на её редактирование.');

        // eslint-disable-next-line
        const time = ms(args[2]);
        if (!time) return getHelp(message, this.name);
        if (time < 300000 || time > 1000 * 60 * 60 * 24) return message.fail('Время задержки должно быть между **5-тью минутами** и **одними сутками**.');
      ...
      default:
        return message.channel.send(
          new MessageEmbed()
            ...
            .addField('Настройка', 'Подробную справку по настройке задержек для команд вы сможете получить на [этой странице](https://docs.flamebot.ru/settings/economy).', true)
