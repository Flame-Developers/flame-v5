  ...
  async run(message, args) {
    ...
    switch (option) {
      ...
      case 'role':
        ...
        if (!role) return message.reply('Пожалуйста укажите роль для новых пользователей :no_entry:');
        if (!message.guild.roles.cache.has(role.id)) return message.reply('Указанной роли не существует на данном сервере :no_entry:');
        if (message.guild.me.roles.highest.position < role.position) return message.reply('Я не смогу выдавать данную роль, так как она находится выше моей на данном сервере :no_entry:');
        message.client.database.collection('guilds').updateOne({ guildID: message.guild.id }, {
          $set: {
            welcomeRole: role.id,
          },
        ...
        break;
      case 'channel':
        if (!data.welcome.enabled) return message.reply('На данном сервере ещё не включена система приветствий. Включите её, прежде чем настраивать другие параметры :no_entry:');
        // eslint-disable-next-line no-case-declarations
      ...
      default:
        return message.channel.send(
          new MessageEmbed()
            ...
            .addField('Сообщение приветствия', data.welcome.text ? `\`\`\`${data.welcome.text}\`\`\`` : 'Сообщение приветствия не установлено.')
            .addField('Роль для новых пользователей', data.welcomeRole ? `<@%${data.welcomeRole}> (${data.welcomeRole})` : 'Роль для новых поьзователей не установлена.')
